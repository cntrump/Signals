...
public final class ThreadPoolTask {
    ...
    private let action: (ThreadPoolTaskState) -> Void

    public init(_ action: @escaping(ThreadPoolTaskState) -> Void) {
        self.action = action
    }
...
@objc public final class ThreadPool: NSObject {
    ...
    fileprivate func workOnQueue(_ queue: ThreadPoolQueue, action: () -> Void) {
